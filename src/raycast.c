...
double		find_wall(t_env *e, t_map *map, t_intersect int_p)
{
	...
	if (x_inters.dist < y_inters.dist)
	{
		int_p = x_inters;
		int_p.side = 0;
		tex = IN_RANGE(RAD_TO_DEG(int_p.ang), 0, 180) ? e->tex[(int_p.tex \
		* 4)] : e->tex[(int_p.tex * 4 + 2)];
	}
	else
	{
		int_p = y_inters;
		...
		e->tex[(int_p.tex * 4 + 1)] : e->tex[(int_p.tex * 4 + 3)];
	}
	int_p.dist *= cos(fabs(DEG_TO_RAD(map->vw_ang) - int_p.ang));
	draw_wall(e, map, &int_p, tex);
	map_rays(e, map, &int_p);
	return (1);
}
